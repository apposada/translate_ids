#!/bin/bash
# Usage: ./translate_ids.sh INPUT.Fasta
set -e
...
speciesnames=${1##*/} # grabs the new id from the name of the fasta. Should be optional: if $2 (==speciesname) specified, then $speciesname=$2
...
awk -v var1=$speciesnames -v var2=$numdigits 'BEGIN {S=var1} {D=var2} {OFS="\t"}\
{ print $0, ">"S"_"} {printf "%05d\n",NR }'\
${file}.col1 | \
paste - - | \
perl -pe "s/_\t/_/" | \
perl -pe "s/^/\>/" > ${speciesnames}_dictionary.dct
